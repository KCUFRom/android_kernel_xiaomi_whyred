...
export KBUILD_COMPILER_STRING=$(~/clang9/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
CLANG_PATH=~/clang9/bin/clang
CROSS_COMPILE=~/linaro7/bin/aarch64-linux-gnu-
